...
public class Fait {
	...
	public Fait(Relation relation, char[] variables, Homomorphism homomorphism, int cpt) {
	...
	public Fait(Relation relation, char[] values, int cpt) {
		this.relation = relation;
		this.values = values;
		this.cpt = cpt;
	}

	public static Fait createBasicFait(Relation relation, int cpt) {
		char[] variables = new char[relation.getArity()];
		for (int i = 0; i < relation.getArity(); i++) {
			variables[i] = Value.basicVar;
		}

		return new Fait(relation, variables, cpt);
	}
	 ...
	 */
	public static List<Fait> createFaitsForMatching(InstanceSchema instanceSchema, Constraint constraint, int cpt) {
		...
		for (Homomorphism homomorphism : homomorphisms) {
			...
			for(RelationName relationName : constraintElementGroup.keySet()){
				for(int i=0;i<constraintElementGroup.get(relationName).length;i++){
					...
					newFacts.add(Fait.createFaitForMatching(instanceSchema.getSchema().getRelation(relationName), constraintElement, homomorphism, cpt));
				}
			}
		}
	...
	public static Fait createFaitForMatching(Relation relation, ConstraintElement constraintElement, Homomorphism homomorphism, int cpt){
		return new Fait(relation, constraintElement.getVariables(), homomorphism, cpt);
	...
	public String toString() {
		...
		return string += values[values.length - 1] + ") : " + cpt;
	}
}
